...
to render-play
  ...
  [
    ...
    ; register buttons
    register-button [[] -> ( set page "MENU" )] world-size * -1 world-size - btn-height "BACK"
    register-button [[] -> ( play-animation )] world-size * -1 + btn-width * 1.2 world-size - btn-height "PLAY"
  ]
...
to create-frame
  ...
  ; takes current drawing field and converts it into a frame ( disregard the top 20 pixels )
  ask patches with [pycor < world-size - btn-height and pcolor = 3] [ set pcolor black ]
  let x world-size * -1

  let frame (list world-size (world-size - btn-height)) ; width, height

  while [x <= world-size] [
    let y world-size * -1
    while [ y < world-size - btn-height ] [
      set frame lput ([pcolor] of patch x y) frame
      if [pcolor] of patch x y != 0
      [ ask patch x y [ set pcolor 3 ] ]
...
to listen-animate
  ...
  ifelse mouse-ycor < world-size - btn-height
  [
    ...
    [
      ...
      ask patch mouse-xcor mouse-ycor [
        ...
        ask neighbors [
          if pycor < world-size - btn-height
          [
            set pcolor draw-color
          ]
        ]
...
to play-animation
  foreach animation [frame ->
    ask patches with [ pcolor != 0 and pycor < world-size - btn-height] [ set pcolor black ]

    let width item 1 frame
    let height item 2 frame

    let y 2
    while [ y < length frame ] [
      if item y frame != 0 [
        ask patch ((floor ((y - 2) / (world-size * 2) )) - 200) (((y - 2) mod (world-size * 2 - btn-height)) - world-size + btn-height) [ set pcolor item y frame ]
      ]
      set y y + 1
    ]
...
to render-animator
  ...
  [
    ...
    register-button [[] -> ( set page "MENU" )] world-size * -1 world-size - btn-height "BACK"
    register-button [[] -> ( create-frame )] world-size * -1 + btn-width * 1.2 world-size - btn-height "KEYFRAME"
    register-button [[] -> ( set draw-color 0 )] world-size * -1 + btn-width * 2.5 world-size - btn-height "ERASE"
    register-button [[] -> ( ask patches with [pycor < world-size - btn-height] [ set pcolor black ] )] world-size * -1 + btn-width * 3.7 world-size - btn-height "CLEAR"
    register-button [[] -> ( output-animation )] world-size * -1 + btn-width * 4.9 world-size - btn-height "GENERATE"
    ...
    ; render color-picker
    ask patches with [pxcor >= 60 and pycor >= world-size - btn-height] [set pcolor pxcor]
  ]
